...
mkdir $results_path
mkdir $results_path"/results_final_union"
...
start_ind=$(($array*8010))
end_ind=$((($array+1)*8010 ))
...
#filenames=( $(cut -d ',' -f1 ../../TFBS/Results/tomtom/tomtom/filenames.csv ) )
  filenames=( $(cut -d ',' -f1 ../../TFBS/PWMs_final_union/filenames.csv ) )
  length=${#filenames[@]}
  TF_PATH=../../TFBS/
  TF1=${filenames[$i]}
  TF2=${filenames[$j]}
  ...
  motifnames=( $(cut -d ',' -f1 ../../TFBS/PWMs_final_union/motifnames.csv ) )
...
cd $LOCAL_SCRATCH
ls
cp "result_"$array".out" $results_path"/results_final_union/"
