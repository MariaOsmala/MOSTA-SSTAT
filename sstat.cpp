...
{
  const string cswrongargs("Insufficient parameters\nCall sstat <gc> <[list:]transfac-file> <threshold-method> <threshold-parameter> [<partial-execution>] [<return diagonal>]\n");
  //check command line
  if (argc<5)
    {
      cout << cswrongargs;
  ...
  CSimilarityMatrix osm(sflist,gc,stmethod,t,bdiagonal);
