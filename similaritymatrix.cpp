...
CSimilarityMatrix::CSimilarityMatrix(string &asflist,double &gc,string &astmethod,double &at,bool bdiagonal) : sflist(asflist),stmethod(astmethod),igc(gc),t(at),bfilldiagonal(bdiagonal), vopfm(asflist,gc,1)
{
  if (vopfm.size()<2)
    {
...
void CSimilarityMatrix::fill_matrix(int akpartial,bool bfilltwice)
{
  ...
  for (int i=pfmoffset;i<vopfm.size();i++)
    {
      vopfm[i+pfmoffset].adjust_t(stmethod,t);
      valpha.push_back(vopfm[i+pfmoffset].get_alpha());
    }
		  ...
		  for (int j=i+jplus;j<nc;j++)
		    {
		      ...
		      CSimStat osstat(vopfm[i+pfmoffset],vopfm[j+pfmoffset]);
